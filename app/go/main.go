...
func initialize(c echo.Context) error {
	...
	if err != nil {
		c.Logger().Errorf("Failed to start pprotein : %v", err)
		return err
	}
...
func searchChairs(c echo.Context) error {
	...
	conditions = append(conditions, "in_stock = true")
...
func buyChair(c echo.Context) error {
	...
	var chair Chair
	err = tx.QueryRowx("SELECT * FROM chair WHERE id = ? AND in_stock = true FOR UPDATE", id).StructScan(&chair)
	if err != nil {
		if err == sql.ErrNoRows {
...
func getLowPricedChair(c echo.Context) error {
	var chairs []Chair
	query := `SELECT * FROM chair WHERE in_stock = true ORDER BY price ASC, id ASC LIMIT ?`
...
func searchEstateNazotte(c echo.Context) error {
	...
	query := fmt.Sprintf(
		`SELECT * FROM estate WHERE ST_Contains(ST_GeomFromText(''), POINT(latitude, longitude)) ORDER BY popularity DESC, id LIMIT ?`,
